...
public class TxtUtils {
    ...
    public static void setIconInTxtView(TextView txtView, String texto, ArrayList<String> listChars,
        ...
        if (listChars.size() == listImagenes.size()) {
            ...
            for (int i = 0; i < listChars.size(); i++) {
                ...
                spannable.setSpan(new ImageSpan(context, (listImagenes.get(i))), index, index + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }

            txtView.setText(spannable);

        } else {
            Toast.makeText(context, "Debe pasar el mismo numero de caracteres e imagenes", Toast.LENGTH_SHORT).show();
        }
